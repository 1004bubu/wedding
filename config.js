...
window.WEDDING_CONFIG = {
  ...
  images: {
    ...
    // 넘버 세트별로 한 행(열)에 배치됩니다.
    //  · N-1, N-2 두 장인 세트 → 한 행에 2장
    //  · N-1 한 장뿐인 세트(1, 15) → 한 행 전체 너비 1장
    // (그룹 행 배치는 파일명 'N-M' 규칙을 기준으로 app.js가 자동 처리)
    gallery: [
    ...
    // 사진별 보여줄 영역 조정 (object-position). 기본은 중앙('50% 50%').
    //  · 세로 두 번째 값을 키울수록(→100%) 사진의 '아래쪽'을 더 보여줌 (얼굴이 위로 잘릴 때)
    //  · 작게(→0%) 하면 '위쪽'을 더 보여줌
    //  키: 파일명(확장자 제외). 적지 않은 사진은 중앙 그대로.
    galleryFocus: {
      '2-1': '50% 30%',
      '2-2': '50% 30%',
      '5-1': '50% 0%',
  ...
  /* ── 영상(선택) ────────────────────────────────────────────────────
   *  mp4 경로를 넣으면 갤러리 아래에 영상 섹션이 표시됩니다. 비우면 섹션 숨김.
   *  용량이 크므로 탭하기 전에는 영상을 내려받지 않습니다(첫 장면만 표시).
   *  posterTime : 썸네일로 쓸 영상 시점(초). 인트로가 어두우면 값을 키우세요.
   *  poster     : 썸네일 이미지를 직접 지정하고 싶을 때만 사용 (지정 시 posterTime 무시) */
  video: {
    src: 'video/invite_video_web.mp4',
    posterTime: 1.3,
    poster: 'images/video-poster.jpg',
    caption: '탭하면 영상이 재생됩니다 · 소리가 함께 나와요',
  },
